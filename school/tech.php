...
$r=mysqli_query($conn,"SELECT * FROM `een` WHERE chass='$cl'" ); 
$r2=mysqli_query($conn,"SELECT * FROM `een` WHERE id= '$ed'"); 
...
<meta  name="viewport" content="width=device-width,intial-scale=1.0"/> 
    ...
    <body >
      <div ><center>
      <a href="one.php"><button style="width:30%;height:40px;color:#ffff00;background-color:#0f0f00;font-size:15px;"> الصفحة الرئيسية</button></a>
...
$sql = "UPDATE een SET h='$lk'  WHERE id='$row[0]' ";
if ($conn->query($sql) === TRUE) {
echo "Record updated successfully";
header("Location:one.php?edit=33");
} else {
...
$sql = "UPDATE een SET i='$ex'  WHERE id='$row[0]' ";
if ($conn->query($sql) === TRUE) {
echo "Record updated successfully";
header("Location:one.php?edit=33");
} else {
...
<form action="" method="post" enctype="multipart/form-data">
       ...
       <td> <input type="text" name="a" id="te"value="<?php echo $row[1]; ?>" style="width:170px; font-size:20px;"></td>
             ...
             <td style="width:3%; font-size:20px;"><?php echo $row[0];?></td> 
              ...
              <td><?php echo $row[10];?></td> 
               <td><?php echo $row[11];?></td> 
                <td style="width:3%; font-size:20px;"><?php echo $row[12];?></td>
                <td><?php echo $row[13];?></td>
                <td style="background-color:#00ff00;color:#ffffff;"><a href="tech.php?edit=<?php echo $row['id'];?>">اختيار</a></td>
             <?php echo "</tr>";}?>
<?php
if(isset($_GET['edit'])){
}
...
$sql = "UPDATE een SET a='$aa',c='$cc' , d='$dd' , f='$ff',dd='$gg' , h='$hh',i='$ii' , g='$jj',k='$kk'  WHERE id=$id ";
if ($conn->query($sql) === TRUE) {
echo "Record updated successfully";
} else {
echo "Error updating record: " . $conn->error;
